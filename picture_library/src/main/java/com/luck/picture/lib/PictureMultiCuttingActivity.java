...
import com.luck.picture.lib.ucrop.PicturePhotoGalleryAdapter;
...
public class PictureMultiCuttingActivity extends UCropActivity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // 自定义裁剪输出名
        renameCropFilename = intent.getStringExtra(EXTRA_RENAME_CROP_FILENAME);
        // 是否单独拍照
        isCamera = intent.getBooleanExtra(EXTRA_CAMERA, false);
        // 是否混选模式
        isWithVideoImage = intent.getBooleanExtra(EXTRA_WITH_VIDEO_IMAGE, false);
        // 裁剪数据
        list = getIntent().getParcelableArrayListExtra(EXTRA_CUT_CROP);
        // 列表是否显示动画效果
        isAnimation = getIntent().getBooleanExtra(EXTRA_MULTIPLE_RECYCLERANIMATION, true);
        // Crop cut list
    ...
    private void addPhotoRecyclerView() {
        boolean isMultipleSkipCrop = getIntent().getBooleanExtra(EXTRA_SKIP_MULTIPLE_CROP, true);
    ...
    protected void resetCutData() {
        ...
        extras.putParcelable(UCrop.EXTRA_OUTPUT_URI,
                Uri.fromFile(new File(file,
                        TextUtils.isEmpty(renameCropFilename) ? this.getCreateFileName("IMG_CROP_") + suffix : isCamera ? renameCropFilename : this.rename(renameCropFilename))));
        ...
        try {
            ...
            if (cutIndex >= list.size()) {
                setResult(RESULT_OK, new Intent()
                        .putExtra(EXTRA_OUTPUT_URI_LIST, list)
                );
                onBackPressed();
            } else {
                resetCutData();
